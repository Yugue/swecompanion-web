...
import { rrpParts, rrpPartsById } from "@/lib/rrpStudyData";
import { TopBar } from "@/components/TopBar";
import { RRP_PATH } from "@/lib/mlDomains";
import { QuizSection } from "@/components/quiz/QuizSection";

export function generateStaticParams() {
  return rrpParts.filter((part) => part.quizQuestionCount > 0).map((part) => ({ id: part.id }));
}
...
export async function generateMetadata({
  ...
  const part = rrpPartsById[id];
  if (!part) return {};
  return {
    title: `${part.title} Mock Interview Quiz`,
    description: `${part.quizQuestionCount} realistic interview-style questions covering ranking and recommendation ${part.title.toLowerCase()}, written to match a real L4-L6 interview. Premium.`,
    alternates: { canonical: `${RRP_PATH}/${id}/quiz` },
  };
}

export default async function RrpChapterQuizPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  return (
    <div className="flex min-h-screen flex-col" style={{ "--accent": part.color } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-8">
        <Link href={`${RRP_PATH}#${part.id}`} className="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--accent)]">
        ...
        <QuizSection
          ...
          // identically-named chapter ids (every track has a `foundations`).
          quizId={`rrp-${part.id}`}
