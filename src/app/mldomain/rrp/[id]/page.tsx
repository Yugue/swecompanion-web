...
export function generateStaticParams() {
  return rrpParts.flatMap((part) => part.topics.map((topic) => ({ id: topic.id })));
}
...
export async function generateMetadata({
  ...
  const topic = rrpTopicsById[id];
  if (!topic) return {};
  return {
    title: topic.title,
    description: topic.summary,
    alternates: { canonical: `${RRP_PATH}/${id}` },
    openGraph: { title: topic.title, description: topic.summary },
  };
}

export default async function RrpLessonPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const topic = rrpTopicsById[id];
  if (!topic || !lessonExists(id, "rrp")) notFound();
  const partId = rrpPartIdForTopic[id];
  const part = rrpPartsById[partId];
    ...
    <div className="flex min-h-screen flex-col" style={{ "--accent": part.color } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-8">
        ...
        <div className="mb-6 flex items-center gap-3">
          <LessonCompleteToggle topicId={topic.id} accent={part.color} track="rrp" />
        </div>
        ...
        <LessonView topicId={topic.id} track="rrp" />

        {part.quizQuestionCount > 0 && (
          <QuizTeaser
            href={`${RRP_PATH}/${part.id}/quiz`}
