...
export function generateStaticParams() {
  return amlParts.filter((part) => part.quizQuestionCount > 0).map((part) => ({ id: part.id }));
}
...
export default async function AppliedMlChapterQuizPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  return (
    <div className="flex min-h-screen flex-col" style={{ "--accent": part.color } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-8">
        ...
        <QuizSection
          // Prefixed per track, so this chapter's quiz never collides with the deep-learning
          // guide's identically-named chapter ids (e.g. `ml-foundations` vs `aml-foundations`).
          quizId={`aml-${part.id}`}
